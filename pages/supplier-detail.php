    ...
    <div class="container-scroller">
        ...
        <div class="page-body-wraper">
            ...
            ?>
                ...
                <hr class="bottom">
                <?php $id=$_GET['id'];
                $sqlcus ="SELECT   `category`.`name`,`category`.`color`,`category`.`quantity`,`bolt`.`date` 
                 FROM `category` INNER JOIN `supplier`INNER JOIN `bolt` WHERE `supplier`.`sup_code`=`category`.`sup_code` AND `category`.`cate_code`=`bolt`.`cate_code` AND`supplier`.`sup_code`=$id;";
