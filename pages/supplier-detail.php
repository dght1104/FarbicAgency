...
<body>
    <div class="container-scroller">
        ...
        <div class="page-body-wraper">
            ...
            <?php 
                $id=$_GET['id'];
                $sqlsupplier="SELECT * FROM `supplier` WHERE `supplier`.`sup_code`=$id";
            ...
            ?>
                ...
                <hr class="bottom">
                <?php $id=$_GET['id'];
                $sqlcus ="SELECT `category`.`name`,`category`.`color`,`category`.`quantity`,`bolt`.`date`, `currentprice`.`cate_price` FROM `category` INNER JOIN `supplier` INNER JOIN `currentprice` INNER JOIN `bolt` WHERE `supplier`.`sup_code`=`category`.`sup_code` AND `category`.`cate_code`=`bolt`.`cate_code` AND `category`.`cate_code`=`currentprice`.`cate_code` AND`supplier`.`sup_code`=$id;";
